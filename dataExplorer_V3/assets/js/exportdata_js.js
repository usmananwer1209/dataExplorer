...
var loaderSrc = "assets/img/loader.gif";
...
function DataSave(selectId) {
    ...
    if (selectId == 'SelectedComp') {
        ...
        $('select[name="SelectedKpi"] option').each(function (i) {
            values += $(this).val() + ",";
        });
    }
...
function getTable() {
    ...
    if (Anual == 0 && Quarterly == 0) {
       ...
       // $("#DataTable tbody").html("");
        var loadingdata = "<img width='17px' height='17px' style='margin:11px' src='" + loaderSrc + "'/>";
         $(".tableLoader").html(loadingdata);
